...
function ke_initCallback(keConfig) {
    ke_Config = keConfig;
    if (ke_Config.Enabled && ke_checkUserEnabled() && ke_checkUserDisabled() === false) {
...
function ke_checkUserEnabled() {
    if (ke_Config === null || ke_Config.EnabledUserNames === null || ke_Config.EnabledUserNames.length === 0 || ke_Config.EnabledUserNames.indexOf(ke_UserName) !== -1)
        return true;
...
function ke_checkUserDisabled() {
    if (ke_Config === null || ke_Config.DisabledUserNames === null || ke_Config.DisabledUserNames.length === 0 || ke_Config.DisabledUserNames.indexOf(ke_UserName) === -1)
        return false;

    return true;
}

function ke_getAPIDataAsync(queryStringParams, refreshData, callback, returnJSONObject) {
    var innerCallback = function () { ke_getDataAsync(queryStringParams, refreshData, callback, returnJSONObject); };
...
function ke_postAPIDataAsync(queryStringParams, refreshData, callback, returnJSONObject, requestBody) {
    var innerCallback = function () { ke_postDataAsync(queryStringParams, refreshData, callback, returnJSONObject, requestBody); };
    ke_checkLocalStorageCacheList(innerCallback);
}
...
function ke_checkLocalStorageCacheList(callback) {
    ...
    if (cacheListJSON != null) {
        ...
        if (msDifference > cacheListRefreshFrequency) {
            ...
            ke_log("refresh cache list (" + Math.floor((msDifference / 1000)) + " seconds since last update)");
...
function ke_getItemFromCacheList(clientKey) {
    var cacheListJSON = localStorage.getItem("data=cachelist&userid=" + ke_UserID);
    var cacheList = JSON.parse(cacheListJSON);
...
function ke_getDataAsync(queryStringParams, refreshServerCache, callback, returnJSONObject) {
    var URL = ke_APIURL + "?" + queryStringParams;
    var clientKey = queryStringParams;

    var cachedDataJSON = localStorage.getItem(clientKey);
    if (refreshServerCache == false && cachedDataJSON != null && cachedDataJSON != "") {
...
function ke_postDataAsync(queryStringParams, refreshServerCache, callback, returnJSONObject, requestBody) {
    var URL = ke_APIURL + "?" + queryStringParams;
    var clientKey = queryStringParams + '&requestbodyhash=' + ke_hash(requestBody);
...
function ke_xhrSuccess() {
    ...
    if (this.status !== 200 || this.responseText === "") {
        var message = this.responseText !== "" ? this.responseText : "API endpoint returned status code " + this.status;
        var displayMessage = "<br><span class='ke-ko-error-message'>" + message + "</span>";
        ke_ko_commandComplete(displayMessage); //TODO: throw exception instead of displaying message > throw message;
        return;
    }

    var requestURL = this.responseURL.replace("&refreshdata=true", "").replace("?refreshdata=true", "");
    ...
    if (data !== "executequery")
        ke_setLocalStorage(clientKey, this.responseText);
...
function ke_getSettingsValue(settingsCategory, keyName, returnJSONObject) {
    ...
    var result;
    for (var i = 0; i < settingsCategory.Categories.length; i++) {
        result = arguments.callee(settingsCategory.Categories[i], keyName, returnJSONObject);
        if (result != undefined) {
            return result;
        }
    }
...
function ke_log(message, outputpathname) {
    if (outputpathname == undefined) { outputpathname = false; }
    ...
    if (callingFunction.startsWith("ke_") && ke_Config != null && ke_Config.ConsoleLogging == false) {
        return;
    }
...
function ke_getHash(inputString) {
    return inputString.split("").reduce(function (a, b) { a = ((a << 5) - a) + b.charCodeAt(0); return a & a }, 0);
}
...
function ke_eb_init() {
    ...
    if (masterContainer == undefined) { return; }
    ...
    if (extConfig.Enabled == false) { return; }
...
function ke_sb_observeDashboardUpdates() {
    ...
    if (window.location.href.indexOf('/CMSModules/ApplicationDashboard/ApplicationDashboard.aspx') != -1) {
        ...
        var mutationObserver = new MutationObserver
            ...
            function (mutations) {
                ...
                mutations.forEach(
                    function (mutation) {
                        if (buttonEventBound) return;
                        var editButton = document.getElementsByClassName("btn btn-edit-mode btn-default icon-only")[0];
        ...
        mutationObserver.observe(dashboardContainer, { childList: true, subtree: true });
    }
}
...
function ke_sb_loadCallback(shortcutBarItems) {
    ...
    konsoleItem.href = "#";
    konsoleItem.addEventListener("click", function () { ke_ko_showHide(); }, false);
    ...
    refreshItem.href = "#";
    refreshItem.addEventListener("click", function () { ke_sb_refresh(true) }, false);
    ...
    staticResourcesVersionsItem.className = "kentico-extensions-nav-bar-staticresources";
    staticResourcesVersionsItem.innerHTML = "Static Resources";
    staticResourcesVersionsItem.href = window.location.protocol + "//" + window.location.hostname.replace("cms", "site") + "/api/staticresources/remote/versions";
    ...
    liveSiteItem.href = window.location.protocol + "//" + window.location.hostname.replace("cms", "site");
...
function ke_sb_build(shortcutBarItems) {
    ...
    for (var i = 0; i < shortcutBarItems.length; i++) {
        ...
        shortcutLink.id = "kentico-extensions-nav-bar-" + shortcutBarItems[i].name.toLowerCase().replace(' ', '-');
        ...
        shortcutLink.onmouseover = function () {
            var navbarinnerlabel = document.getElementById('kentico-extensions-nav-bar-inner-label');
            navbarinnerlabel.innerHTML = this.name;
            var navbarlabel = document.getElementById('kentico-extensions-nav-bar-label');
        ...
        shortcutIcon.className = shortcutBarItems[i].iconClass + " cms-nav-icon-medium icon-" + shortcutBarItems[i].iconColor.toLowerCase();
...
function ke_sb_hideShow() {
    ...
    var elementToHideShow = document.getElementById("kentico-extensions-nav-bar-shortcuts");
    if (elementToHideShow == undefined) {
        elementToHideShow = document.getElementById("kentico-extensions-nav-bar-message");
    }
    if (elementToHideShow == undefined) return;

    var hideShowLink = document.getElementById("kentico-extensions-nav-bar-hideshow");
...
function ke_sb_showMenu() {
    document.getElementById("kentico-extensions-nav-bar-menu-content").style.display = "block";
}

function ke_sb_hideMenu() {
    document.getElementById("kentico-extensions-nav-bar-menu-content").style.display = "none";
}
...
function ke_uir_getButtons() {
    ...
    if (window.location.href.indexOf('EventLog.aspx') !== -1 && document.querySelectorAll("button[value='Clear log']").length != -1) {
        ke_uir_buttonArray.push(document.querySelectorAll("button[value='Clear log']")[0]);
    }
    ...
    if (window.location.href.indexOf('/Staging/Tools/Tasks/Tasks.aspx') !== -1 ||
        ...
        var syncSubtree = document.querySelectorAll("button[value='Synchronize current subtree']")[0];
        if (syncSubtree !== undefined) {
            ke_uir_buttonArray.push(syncSubtree);
        }
        var syncAll = document.querySelectorAll("button[value='Run complete synchronization']")[0];
...
function ke_uir_disableButtons() {
    ...
    for (var i = 0; i < ke_uir_buttonArray.length; i++) {
        ...
        ke_uir_buttonArray[i].title = "This has been disabled by Kentico Extensions for your safety :)";
    }
}
...
function ke_uir_enableButtonsCallback(userRoles) {
    if (userRoles.filter(r => { return r.RoleDisplayName === "CMS UI Restriction Override" }).length !== 0) {
...
function ke_stuf_init() {
    ...
    if (window.location.href.indexOf("/CMSModules/Staging/Tools/AllTasks/Tasks.aspx") == -1 &&
        window.location.href.indexOf("/CMSModules/Staging/Tools/Tasks/Tasks.aspx") == -1 &&
        window.location.href.indexOf("/CMSModules/Staging/Tools/Objects/Tasks.aspx") == -1 &&
        window.location.href.indexOf("/CMSModules/Staging/Tools/Data/Tasks.aspx") == -1)
        return;
...
function ke_stuf_showLocalDateTime() {
    if (ke_stuf_convertTimeToLocal === false || ke_PreferredUICultureCode !== 'en-AU')
        return;

    var taskTable = document.querySelector("table[id^='m_c_']");
    var taskTableRows = taskTable.getElementsByTagName('tr');
    var timeSpan = taskTableRows[0].getElementsByTagName('th')[4].getElementsByClassName('unigrid-sort-label')[0];
    timeSpan.innerText += "*";
    timeSpan.setAttribute("title", "Kentico Extensions has modified the date/times for time zone: " + Intl.DateTimeFormat().resolvedOptions().timeZone);
    for (var i = 1; i < taskTableRows.length; i++) {
        ...
        var localstr = year + '-' + month + '-' + day + ' ' + utcDateTime.substring(startOfTime) + ' UTC';
...
function ke_stuf_initUserColumn(refreshData) {
    var serverSelect = document.querySelectorAll("select[id$='drpSingleSelect']")[0];
...
function ke_stuf_addSelectAllCheckBox() {
    var kenticoSelectAllCheckbox = document.querySelector("input[id$='headerBox']");
...
function ke_stuf_applyFilter() {
    ...
    if (selectedUserID != 0) {
        ...
        document.getElementById('kentico-extensions-select-all-label').style.display = '';
        ...
        document.getElementById('kentico-extensions-select-all-label').style.display = 'none';
...
function ke_stuf_selectAll() {
    var selectAll = document.getElementById('kentico-extensions-select-all').checked;
    var userSelect = document.getElementById('kentico-extensions-user-filter');
...
function ke_ms_init() {
    ...
    if (mediaSelectors.length == 0) { return; }
    ...
    var mutationObserver = new MutationObserver
        ...
        function (mutations) {
            mutations.forEach(
                function (mutation) {
                    if (mutation.addedNodes.length == 1 && mutation.addedNodes[0].id != undefined && mutation.addedNodes[0].id.indexOf("mediaSelector") > 0) {
...
function ke_ms_addImageSizeLabel(ms) {
    ...
    if (msimage !== undefined) {
        ...
        var qsParams = "data=mediafileinfo&fileguid=" + fileguid + "&width=" + width + "&height=" + height;
...
function ke_ms_addImageSizeLabelCallback(mediaFile) {
    ...
    var labeltext = '<br><b>Size:</b> ' + imagedimensions + ' (' + ke_formatBytes(mediaFile.Size) + ')';
    var detailshtml = mediaFile.Title !== '' ? '<br><b>Title:</b> ' + mediaFile.Title : '';
    detailshtml += mediaFile.Description !== '' ? '<br><b>Alt Text:</b> ' + mediaFile.Description : '';

    var msimage = document.querySelectorAll("img[src*='" + mediaFile.GUID + "'")[0].parentNode;
    ...
    if (mslabel != undefined) {
        ...
        msimage.innerHTML += '&nbsp<div class="ke-ms-label" title="Information provided by Kentico Extensions :)">' + labeltext + detailshtml + '</div>';
    }
}
...
function ke_ti_init() {
    ...
    if (window.location.href.indexOf("/CMSModules/Content/CMSDesk/Default.aspx") == -1 ||
...
function ke_ti_load() {
    ...
    var contentTree = document.querySelectorAll("div[id$='contentcontrolpanel']")[0];
...
function ke_ti_loadCallback(treeItems) {
    ...
    for (var i = 0; i < spanElements.length; i++) {
        ...
        if (infoDiv == undefined) {
            ...
            infoDiv.innerHTML = "<strong>Node Name:</strong> " + treeItem.NodeName + "<br />";
            infoDiv.innerHTML += "<strong>Node ID:</strong> " + treeItem.NodeID + "<br />";
            infoDiv.innerHTML += "<strong>Node GUID:</strong> " + treeItem.NodeGUID + "<br />";
            infoDiv.innerHTML += "<strong>Node Alias Path:</strong> <a target='_blank' href='" + treeItem.AbsolutePath + "'>" + treeItem.NodeAliasPath + "</a><br />";
            infoDiv.innerHTML += "<strong>Page Type:</strong> " + treeItem.ClassDisplayName + " [" + treeItem.ClassName + "]<br />";
            infoDiv.innerHTML += "<strong>Page Template:</strong> " + treeItem.PageTemplateDisplayName + " [" + treeItem.PageTemplateCodeName + "]";
        }
        ...
        parentElement.onmouseover = function () {
            ...
            var contentTreeSplitterRect = document.getElementsByClassName("ui-layout-resizer-west")[0].getBoundingClientRect();
        ...
        parentElement.onmouseout = function () {
            ...
            setTimeout(function () {
                if (document.getElementById("ke_ti_node_" + nodeID).style.display == "") {
            document.getElementById("ke_ti_node_" + nodeID).style.display = "none";
                }
            }, 3000);
        };
    }
...
function ke_di_init() {
    ...
    if (window.location.href.indexOf("/CMSModules/AdminControls/Pages/UIPage.aspx") == -1 ||
...
function ke_di_loadCallback(docItems) {
    ...
    var dateTimeString = docItem.DocumentModifiedWhen.replace("T", " ").substring(0, 19);
    ...
    infoDiv.innerHTML = "<strong>Last modified:</strong> " + docItem.DocumentModifiedBy + " (" + dateTimeString + ")";
    infoDiv.innerHTML += " | <a href='" + docItem.AbsolutePath + " 'target='_blank'>Live Version</a><br />";
...
var ke_ko_mouse_x_pos = 0, ke_ko_mouse_y_pos = 0;
var ke_ko_x_pos = 0, ke_ko_y_pos = 0;
...
function ke_ko_init() {
    ...
    if (kb_ko_config.Enabled === false) { return; }
    ...
    if (ke_getQueryStringValue("showkonsole") === "true" || document.getElementById('konsole') !== null)
        ke_ko_show();
...
function ke_ko_appendElements() {
    ...
    if (masterContainer === undefined) {
        ...
        if (masterContainer === undefined || masterContainer === null)
            return false;
    }
    ...
    konsole.style = "top: 48px; left: 0px; width: " + (window.innerWidth - 18) + "px;" + " display: none;";
    if (masterContainer.id === "konsole")
        konsole.style = "top: 0px; left: 0px; width: " + (window.innerWidth - 18) + "px;";
    ...
    konsoleHeader.ondblclick = function() { ke_ko_dock('headerdblclick'); return false; };
...
function ke_ko_binding() {
    document.getElementById("ke-ko-input").addEventListener("keyup", function (event) {
        ...
        if (event.keyCode == 38) {
            ...
            document.getElementById("ke-ko-input").value = ke_ko_inputhistory[ke_ko_inputhistory.length - ke_ko_inputhistorycount];
            ke_ko_inputhistorycount++;
        }
        if (event.keyCode == 40) {
            ke_ko_inputhistorycount--;
            if (ke_ko_inputhistorycount == 0 || ke_ko_inputhistorycount > ke_ko_inputhistory.length) {
                ke_ko_inputhistorycount = 1;
                return;
            }
            document.getElementById("ke-ko-input").value = ke_ko_inputhistory[ke_ko_inputhistory.length - ke_ko_inputhistorycount];
    ...
    document.getElementById("ke-ko-afterinput").addEventListener("click", function () {
    ...
    document.getElementById("ke-ko-matrix").addEventListener("click", function () {
        ke_ko_hideMatrix();
    });
...
function ke_ko_globalBinding() {
    ...
    document.addEventListener("keyup", function (event) {
        ...
        if (event.keyCode == 120) {
            localStorage.setItem('kenticoextensions-konsole-f9', ke_getCurrentDateTimeString());
            ke_ko_showHide();
        }
        // ESC will hide matrix
        if (event.keyCode == 27) {
            localStorage.setItem('kenticoextensions-konsole-esc', ke_getCurrentDateTimeString());
            ke_ko_hideMatrix();
        }
...
function ke_ko_processInput() {
    try { 
        ...
        ke_log(e.message);
        ke_ko_commandComplete("<br><span class='ke-ko-error-message'>Bugger! Unfortunatley, an error occurred. Check the console log for details.</span>");
    }
}
...
function ke_ko_processInputExecution() {
    ...
    if (ke_ko_inputhistory.length == 0 || inputValue != ke_ko_inputhistory[ke_ko_inputhistory.length - 1]) {
        ke_ko_inputhistory.push(inputValue);
    }

    if (inputValue === "test error")
        throw 'Test Error';

    if (inputValue === "clear")
        outputElement.innerHTML = "";

    if (inputValue.startsWith("dock"))
        ke_ko_dock();

    if (inputValue === "hide")
        ke_ko_hide();

    if (inputValue === "matrix")
        ke_ko_showMatrix();

    if (inputValue === "clear" || inputValue.startsWith("dock") || inputValue === "hide" || inputValue === "matrix") {
    ...
    if (inputValue === "help")
        return ke_ko_showHelp();

    if (ke_ko_eqMode()) {
        if (inputValue === "exit")
            return ke_ko_executeQueryMode(false);
        else
            return ke_ko_executeQuery();
    }
    ...
    if (ke_ko_commandShortcuts(inputValue))
        return;

    if (inputValue.startsWith("ls pt"))
        return ke_ko_listPageTypes();

    if (inputValue.startsWith("ls ct"))
        return ke_ko_listCustomTables();

    if (inputValue.startsWith("ls mc"))
        return ke_ko_listModuleClasses();

    if (inputValue.startsWith("ls dts"))
        return ke_ko_listDatabaseTableSchema();
    else if (inputValue.startsWith("ls dt"))
        return ke_ko_listDatabaseTables();

    if (inputValue.startsWith("ls cs"))
        return ke_ko_listClassSchema();

    if (inputValue.startsWith("ls cosh"))
        return ke_ko_listCommandShortcuts();

    if (inputValue.startsWith("ls si"))
        return ke_ko_listSites();

    if (inputValue.startsWith("ls sk"))
        return ke_ko_listSettingKeys();

    if (inputValue.startsWith("ls td"))
        return ke_ko_listTreeData();

    if (inputValue.startsWith("ls mf"))
        return ke_ko_listMediaFiles();

    if (inputValue.startsWith("ls rp"))
        return ke_ko_listReports();

    if (inputValue.startsWith("ls fm"))
        return ke_ko_listForms();

    if (inputValue.startsWith("eq") && ke_GlobalAdmin === false)
        return ke_ko_commandComplete("<br><span class='ke-ko-error-message'>This command is restricted to Global Admins only<span>");

    if (inputValue.startsWith("eq list"))
        return ke_ko_listConfigQueries();
    if (inputValue.startsWith("eq mode"))
        return ke_ko_executeQueryMode(true);
    if (inputValue.startsWith("eq"))
        return ke_ko_executeQuery();

    if (inputValue.startsWith("ls endpoints"))
        return ke_ko_listEndpoints();

    if (kb_ko_config !== null && kb_ko_config.Config !== undefined && kb_ko_config.Config.endpoints !== undefined && kb_ko_config.Config.endpoints.length !== 0) {
        var commandMatch = kb_ko_config.Config.endpoints.find(c => inputValue.startsWith(c.command));
        if (commandMatch !== undefined) {
            return ke_ko_openURL(commandMatch.endpoint, commandMatch.parameters);
        }
    }

    ke_ko_commandComplete("<br><span class='ke-ko-error-message'>Please check the command and try again. For a full list of available commands type help.</span>");
}

function ke_ko_commandShortcuts(inputValue) {
    if (kb_ko_config === null || kb_ko_config.Config === undefined || kb_ko_config.Config.commandShortcuts === undefined || kb_ko_config.Config.commandShortcuts.length === 0) {
        return false;
    }

    var itemMatch = kb_ko_config.Config.commandShortcuts.find(s => s.shortcut === inputValue);
...
function ke_ko_openURL(urlPath, paramName) {
    var siteURL = window.location.protocol + "//" + window.location.hostname.replace("cms", "site");
    ...
    ke_ko_commandComplete(`<br><span style="color: cyan">Attempting to open ` + urlLink + ` in a new tab.</span>`);
    setTimeout(function () { window.open(fullURL, "_blank") }, 500);
}
...
function ke_ko_commandComplete(output) {
    ...
    if (outputElement.innerHTML.endsWith("<br>") || outputElement.innerHTML == "") {
        outputElement.innerHTML += (eqMode ? "eq>" : "ke>") + inputElement.value;
    } else {
        outputElement.innerHTML += "<br>" + (eqMode ? "eq>" : "ke>") + inputElement.value;
    }
    if (output !== undefined)
...
function ke_ko_showHelp() {
    ...
    html += ke_ko_getHelpTableRow('clear', '', 'clears the konsole output', '', 'clear');
    html += ke_ko_getHelpTableRow('dock', 'full|right|left|reset', 'resizes the konsole and docks to the specified location', '');

    if (ke_GlobalAdmin) {
        html += ke_ko_getHelpTableRow('eq', 'list', 'display the config query list', '!', 'eq list');
        html += ke_ko_getHelpTableRow('eq', 'mode', `execute queries without the eq prefix (type 'exit' to exit)`, '!', 'eq mode');
        html += ke_ko_getHelpTableRow('eq', '{key}', 'execute a query from the config query list', '!');
    ...
    html += ke_ko_getHelpTableRow('ls', 'cosh *', 'list command shortcuts', '', 'ls cosh');
    html += ke_ko_getHelpTableRow('ls', 'ct *', 'list custom tables', '', 'ls ct');
    html += ke_ko_getHelpTableRow('ls', 'dt *', 'list database tables', '', 'ls dt');
    html += ke_ko_getHelpTableRow('ls', 'dts *', 'list database table schema', '', 'ls dts');
    html += ke_ko_getHelpTableRow('ls', 'endpoints *', 'list configured endpoint commands', '', 'ls endpoints');
    html += ke_ko_getHelpTableRow('ls', 'fm *', 'list forms', '', 'ls mc');
    html += ke_ko_getHelpTableRow('ls', 'mc *', 'list modules classes', '', 'ls mc');
    html += ke_ko_getHelpTableRow('ls', 'mf *', 'list media files', '50 results max', 'ls mf');
    ...
    html += ke_ko_getHelpTableRow('matrix', '', 'matrix animation (Esc exits)', '', 'matrix');

    html +=    `</tbody>
                <tfoot>
                    <tr class="ke-ko-table-header">
                        <td colspan="5">` + (ke_GlobalAdmin ? `
...
function ke_ko_getHelpTableRow(command, params, description, restrictions, commandLink) {
    return `<tr>
                <td>` + command + `</td>
                <td>` + params + `</td>
                <td>` + description + `</td>
                <td>` + restrictions + `</td>` +
                (commandLink === undefined || commandLink === '' ? `<td>&nbsp;</td>` :
                `<td><a href="#" onclick="ke_ko_runCommand('` + commandLink + `');return false;">` + commandLink + `</a></td>`) +
            `</tr>`;
}
...
function ke_ko_dock(param) {
    ...
    if (param === "")
        param = konsole.getAttribute("data-dockstatus");

    if (param === "headerdblclick") {
        if (konsole.getAttribute("data-dockstatus") === "full")
            param = "reset";
        else
            param = "full";
    }
    ...
    if (konsoleInput == "dock reset" || param === "reset") {
        konsole.style = "top: 50px; left: 50px; width: 600px; display: inline-block;";
...
function ke_ko_sitesCallback(dataSet) {
    ...
    filteredResults.forEach(function (resultItem) {
        ...
        displayItem.URL = `<a target="_blank" href="` + resultItem.URL + `">` + resultItem.URL + `</a>`;
...
function ke_ko_pageTypesCallback(dataSet) {
    ...
    filteredResults.forEach(function (resultItem){
        var displayItem = new Object();
        displayItem.DisplayName = `<a href="` + resultItem.ClassURL + `" target="_blank">` + resultItem.DisplayName + `</a>`;
        displayItem.CodeName = resultItem.CodeName;
        displayItem.TableName = resultItem.TableName;

        displayItem.InheritsFrom = '&nbsp;';
        if (resultItem.InheritedClassID !== null)
        displayItem.InheritsFrom = `<a href="#" onclick="ke_ko_runCommand('ls pt ` + resultItem.InheritedCodeName + `');return false;">` + resultItem.InheritedCodeName + `</a>`;

        displayItem.SiteIDs = '&nbsp;';
        if (resultItem.SiteIDs !== null)
        displayItem.SiteIDs = resultItem.SiteIDs;

        displayItem.Schemas = `<a href="#" onclick="ke_ko_databaseTableSchemaClick('` + resultItem.TableName + `');return false;">Database</a>` +
            ` | <a href="#" onclick="ke_ko_classSchemaClick('` + resultItem.CodeName + `');return false;">Class</a>`;
        displaySet.push(displayItem);
    });
    ke_ko_displayResultsTable(displaySet);
    return;
...
function ke_ko_customTablesCallback(dataSet) {
    ...
    filteredResults.forEach(function (resultItem){
        ...
        displayItem.CodeName = resultItem.CodeName;
        displayItem.TableName = resultItem.TableName;
        displayItem.SiteIDs = resultItem.SiteIDs !== null ? resultItem.SiteIDs : '&nbsp;';
        displayItem.Links = `<a href="#" onclick="ke_ko_databaseTableSchemaClick('` + resultItem.TableName + `');return false;">Table Schema</a>` +
            ` | <a href="#" onclick="ke_ko_classSchemaClick('` + resultItem.CodeName + `');return false;">Class Schema</a>` +
            ` | ` + ke_ko_getCustomTableLink(resultItem.ClassID, 'Edit Table') +
            ` | <a target="_blank" href="/CMSModules/CustomTables/Tools/CustomTable_Data_List.aspx?objectid=` + resultItem.ClassID + `">Edit Data</a>`;
        displaySet.push(displayItem);
    });
    ke_ko_displayResultsTable(displaySet);
    return;
...
function ke_ko_moduleClassesCallback(dataSet) {
    ...
    filteredResults.forEach(function (resultItem) {
        ...
        displayItem.SiteIDs = resultItem.SiteIDs !== null ? resultItem.SiteIDs : '&nbsp;';
        displayItem.Links = `<a href="#" onclick="ke_ko_databaseTableSchemaClick('` + resultItem.ClassTableName + `');return false;">Table Schema</a>` +
            ` | <a href="#" onclick="ke_ko_classSchemaClick('` + resultItem.ClassName + `');return false;">Class Schema</a>` +
            ` | <a target="_blank" href="/CMSModules/AdminControls/Pages/UIPage.aspx?elementguid=f6dfdcf6-1515-4b50-b16b-78f9e6635005&objectid=` + resultItem.ID + `">Edit Module</a>` + 
...
function ke_ko_getCustomTableLink(objectID, linkText) {
    return `<a href="/CMSModules/AdminControls/Pages/UIPage.aspx?elementguid=5399424e-1200-40a7-a108-e4e86d238a56&objectid=` + objectID + `" target="_blank">` + linkText + `</a>`;
}

function ke_ko_getClassLink(objectID, linkText) {
    return `<a href="/CMSModules/AdminControls/Pages/UIPage.aspx?elementguid=f4c97d07-378a-4693-b9dd-00df3a252d68&objectid=` + objectID + `" target="_blank">` + linkText + `</a>`;
}
...
function ke_ko_databaseTablesCallback(dataSet) {
    ...
    filteredResults.forEach(function (resultItem) {
        ...
        if (resultItem.className !== "")
            displayItem.Schemas += ` | <a href="#" onclick="ke_ko_classSchemaClick('` + resultItem.ClassName + `');return false;">Class</a>`;
        displaySet.push(displayItem);
    });
    ke_ko_displayResultsTable(displaySet);
    return;
...
function ke_ko_listDatabaseTableSchema() {
    ...
    var refreshData = ke_ko_getRefreshDataParamValue();
    ke_getAPIDataAsync(qsParams, refreshData, ke_ko_databaseTableSchemaCallback, true);
}
...
function ke_ko_getInputParamValue(paramName, defaultValue = null) {   
    ...
    var paramMatches = inputParams.filter(r => { return r.name === paramName });
...
function ke_ko_getInputParameters(input) {
    var inputValue = input === undefined ? document.getElementById("ke-ko-input").value : input;
    var params = [];
    if (inputValue === "")
        return params;
    ...
    paramNames.forEach(function (item, index) {
        if (index > 0) {
            var paramName = item.indexOf(' ') === -1 ? item : item.substr(0, item.indexOf(' '));
...
function ke_ko_getInputWildcard(input) {
    ...
    if (inputTokens.length < 3 || inputTokens[2].startsWith('-'))
        return null;
    
    var wildcardStart = ke_ko_nthIndex(inputValue, ' ', 2) + 1;
    var wildcardEnd = inputValue.indexOf('-') !== -1 ? inputValue.indexOf('-') - 1 : inputValue.length;
...
function ke_ko_filterDataset(dataset) {
    var inputParams = ke_ko_getInputParameters();
    var inputParams = inputParams.filter(p => { return p.name !== "refresh" });
    var wildcard = ke_ko_getInputWildcard();

    if (inputParams.length === 0 && wildcard === null)
        return dataset;
    ...
    dataset.forEach(function (record, rowindex) {
        Object.keys(dataset[rowindex]).forEach(function (key) {
            if (record[key] !== null) {
                if (wildcard !== null && record[key].toString().toLowerCase().indexOf(wildcard.toLowerCase()) !== -1) {
                ...
                inputParams.forEach(function (param) {
                    if (key.toLowerCase() === param.name.toLowerCase() && record[key].toString().toLowerCase().indexOf(param.value.toLowerCase()) !== -1) {
                        if (addedIndexes.includes(rowindex) === false){
                            filteredDataset.push(record);
                            addedIndexes.push(rowindex);
                        }
...
function ke_ko_settingKeysCallback(dataSet) {
    ...
    filteredResults.forEach(function (settingKey){
        ...
        if (settingKey.CategoryParentID !== null)
            displayItem.DisplayName = `<a target="_blank" href="` + ke_CMSSiteURL + `CMSModules/Settings/Pages/Keys.aspx?categoryid=` + settingKey.CategoryParentID + `">` + settingKey.DisplayName + `</a>`;
...
function ke_ko_formsCallback(dataSet) {
    ...
    filteredResults.forEach(function (resultItem) {
        ...
        displayItem.Links = `<a href="#" onclick="ke_ko_databaseTableSchemaClick('` + resultItem.TableName + `');return false;">Table Schema</a>` +
            ` | <a href="#" onclick="ke_ko_classSchemaClick('` + resultItem.ClassName + `');return false;">Class Schema</a>` +
            ` | <a target="_blank" href="/CMSModules/AdminControls/Pages/UIPage.aspx?elementguid=0e8d0795-e114-4075-a9f3-226e40bf4207&objectid=` + resultItem.ID + `">Form Admin</a>`;
        displaySet.push(displayItem);
    });
    ke_ko_displayResultsTable(displaySet);
    return;
...
function ke_ko_reportsCallback(dataSet) {
    ...
    filteredResults.forEach(function (result){
        ...
        displayItem.DisplayName = `<a target="_blank" href="` + ke_CMSSiteURL + `CMSModules/Reporting/Tools/Report_View.aspx?parentobjectid=` + result.ReportCategoryID + `&reportid=` + result.ReportID + `&tabslayout=horizontal&objectid=` + result.ReportID + `">` + result.DisplayName + `</a>`;
...
function ke_ko_treeDataCallback(dataSet) {
    ...
    filteredResults.forEach(function (resultItem){
        var displayItem = new Object();
        displayItem.DocumentNamePath = `<a target="_blank" href="` + ke_CMSAdminURL + `?action=edit&nodeid=` + resultItem.NodeID + `&culture=` + resultItem.DocumentCulture + `#95a82f36-9c40-45f0-86f1-39aa44db9a77">` + resultItem.DocumentNamePath + `</a>`; // the GUID is the pages ui element identifier
        displayItem.NodeAliasPath = `<a target="_blank" href="` + ke_PublicSiteURL + resultItem.NodeAliasPath.substr(1) + `">` + resultItem.NodeAliasPath + `</a>`;
        displayItem.ClassName = `<a href="#" onclick="ke_ko_runCommand('ls pt ` + resultItem.ClassName + `');return false;">` + resultItem.ClassName + `</a>`;
        displayItem.DocumentCulture = resultItem.DocumentCulture;
        displayItem.PageTemplateCodeName = resultItem.PageTemplateCodeName === null ? ' ' : resultItem.PageTemplateCodeName;
...
function ke_ko_listConfigQueries() {
    if (kb_ko_config === null || kb_ko_config.Config === undefined || kb_ko_config.Config.queries === undefined || kb_ko_config.Config.queries.length === 0) {
        html = "<br>Konsole configuration does not contain any configured queries<br>";
        ke_ko_commandComplete(html);
        return;
    }
...
function ke_ko_executeQueryMode(enabled) {
    ...
    if (enabled === false && kekoLabel.classList.contains("ke-ko-eq-mode") === true)
    ...
    if (enabled === false && kekoInput.classList.contains("ke-ko-eq-mode") === true)
        kekoInput.classList.remove("ke-ko-eq-mode");
}

function ke_ko_listEndpoints() {
    if (kb_ko_config === null || kb_ko_config.Config === undefined || kb_ko_config.Config.endpoints === undefined || kb_ko_config.Config.endpoints.length === 0) {
        html = "<br>Konsole configuration does not contain any configured endpoints<br>";
        ke_ko_commandComplete(html);
        return;
    }
    ...
    filteredResults.forEach(function (resultItem) {
        ...
        displayItem.Parameters = resultItem.parameters === undefined ? " " : resultItem.parameters;
        displayItem.Description = resultItem.description === undefined ? " " : resultItem.description;
        displayItem.Restriction = resultItem.restriction === undefined ? " " : resultItem.restriction;
        displayItem.QuickRun = `<a href="#" onclick="ke_ko_runCommand('` + resultItem.command + `');return false;">` + resultItem.command + `</a>`;
        displaySet.push(displayItem);
    });
    ke_ko_displayResultsTable(displaySet);
    return;
...
function ke_ko_listCommandShortcuts() {
    if (kb_ko_config === null || kb_ko_config.Config === undefined || kb_ko_config.Config.commandShortcuts === undefined || kb_ko_config.Config.commandShortcuts.length === 0) {
        html = "<br>Konsole configuration does not contain any configured command shortcuts<br>";
        ke_ko_commandComplete(html);
        return;
    }

    var filteredResults = ke_ko_filterDataset(kb_ko_config.Config.commandShortcuts);
    var displaySet = [];
    filteredResults.forEach(function (resultItem) {
        var displayItem = new Object();
...
function ke_ko_executeQuery() {
    ...
    if (kb_ko_config !== null && kb_ko_config.Config !== undefined && kb_ko_config.Config.queries !== undefined) {
        var configQuery = kb_ko_config.Config.queries.filter(r => { return r.key === query })[0];
    ...
    if (query.indexOf(" -verified") === -1 &&
        ...
        ke_ko_commandComplete("<br><span style='color: red'>This command contains high risk keywords. Append -verified to the end of the command to execute.</span>");
...
function ke_ko_displayResultsTable(queryResults) {
    ...
    if (typeof queryResults === 'string') {
        html = `<table class="ke-ko-table">
                    <tbody>                    
                        <tr class="ke-ko-table-header">
                            <td>Result</td>
                        </tr>
                        <tr>
                            <td>` + queryResults.replaceAll("\r\n", "<br>") + `</td>
                        </tr>
                    <tbody>
                </table>`;
    ...
    for (var i = 0; i < queryResults.length; i++) {
        row = queryResults[i];
            ...
            for (var r = 0; r < Object.keys(queryResults[i]).length; r++) {
                ...
                if (cellValue !== null && typeof cellValue === 'string' && cellValue.startsWith("<a ") === false
                        && (cellValue.length > 100 || cellValue.match(/\r/g) !== null || cellValue.match(/\n/g) !== null))
                    html += `<td><textarea class="ko-text-area narrow">` + cellValue + `</textarea></td>`;
                else
...
function ke_ko_loadMatrix() {
    ...
    function ke_ko_draw() {
        ...
        for (var i = 0; i < drops.length; i++) {
            ...
            if (sum >= customtextlength)
                curchar = sum - customtextlength;
            else
                curchar = sum;
